...
country.codes.body <- content(country.codes, "text")
country.codes.data <- fromJSON(country.codes.body)
syria.code <- as.character(country.codes.data %>% filter(name_en == "Syrian Arab Republic") %>% select(country_code))
